...
public class ArgazkienTaula extends JPanel {
	...
	private Object[][] data;
	// private String[] columnNames = { "Titulua", "Deskripzioa",
	// "Data1","Data2","Data3","Data4","Etiketa"};
	...
	public void elementuakGehitu() {
		...
		try {
			...
			for (int errenkada = 0; errenkada < emaitzak.size(); errenkada++) {
					...
					/*
					 * data[errenkada][1] = argazkia[1]; data[errenkada][2] =
					 * argazkia[2]; data[errenkada][3] = argazkia[3];
					 * data[errenkada][4] = argazkia[4]; data[errenkada][5] =
					 * argazkia[5]; data[errenkada][6] = argazkia[6];
					 */		
			}
		} catch (IOException e) {
			e.printStackTrace();
